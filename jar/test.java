...
public class test {
    public class Word{
        String value;
        String separated = syllable(value.replace(",","").replace(".","").replace);
        char pontuation;
        boolean hasUpper;
        boolean skip;
    
    public Word(String value){
    if (this.value.startsWith("*")){this.skip = true;}
    else{this.skip = false;}

    if (this.value.endsWith(".") || this.value.endsWith(",") || this.value.endsWith("?") || 
    this.value.endsWith("!") || this.value.endsWith(":") || this.value.endsWith(";"))
    {this.pontuation = this.value.charAt(this.value.length() -1);}
