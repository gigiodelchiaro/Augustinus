while True:
    ...
    if note == "a":
        ...
        input("Por favor entre uma nota válida")
        continue
    ...
    text_space = text_comma_fix.replace(" ","("+ note +") ")
    text_hyphen = text_space.replace("-","-("+ note +")")



    if line_break == "s":
        text_dot = text_hyphen.replace(".",".("+ note_down +".) (::Z)")
    else:
        text_dot = text_hyphen.replace(".",".("+ note_down +".) (::)")


    if respiration == "s":
        text_comma = text_dot.replace(",",",("+ note_down +".) (,)")
    else:
        text_comma = text_dot.replace(",",",("+ note_down +".)")

    text_down = text_comma.replace("*", "("+ note_down +")")

    text_down_fix = text_down.replace("(" + note + ")("+ note_down +")","("+ note_down +")")
    text_up_fix = text_down_fix.replace("(" + note_down + ")("+ note +")","("+ note_down +")")
    text_down_comma_fix = text_up_fix.replace("("+ note_down +"),("+ note_down +".)",",(" + note_down +".)")
